	...
	*numSkins = 0;
	...
	do
	{
		...
		for( skinptr = listbuf; k > 0; k--, skinptr += strlen( skinptr )+1 )
		{
			...
			// do not accept skins called invisibility
			if( strstr( scratch, "invisibility" ) )
				continue;
...
void UI_PlayerModelBounds( struct model_s *model, vec3_t mins, vec3_t maxs, bonepose_t *boneposes )
{
	vec3_t playerbox_mins = { -16, -16, -24 };
	vec3_t playerbox_maxs = { 16, 16, 40 };
	...
	AddPointToBounds( playerbox_mins, mins, maxs );
	AddPointToBounds( playerbox_maxs, mins, maxs );

	if( !model )
		return;

	trap_R_ModelBounds( model, tmins, tmaxs );
	AddPointToBounds( tmins, mins, maxs );
	AddPointToBounds( tmaxs, mins, maxs );

	if( !boneposes )
		return;

	skel = UI_SkeletonForModel( model );
	if( !skel )
		return;

	UI_SkelposeBounds( skel, boneposes, tmins, tmaxs );
	AddPointToBounds( tmins, mins, maxs );
	AddPointToBounds( tmaxs, mins, maxs );
}
...
void UI_DrawPlayerModel( char *model, char *skin, byte_vec4_t color, int xpos, int ypos, int width, int height, int frame, int oldframe )
{
	...
	UI_PlayerModelBounds( entity.model, mins, maxs, entity.boneposes );
