...
void CG_BoltExplosionMode( vec3_t pos, vec3_t dir, int fire_mode )
{
	...
	if( fire_mode == FIRE_MODE_STRONG )
	{
		...
		CG_ImpactPuffParticles( pos, dir, 8, 1.25f, 1, 1, 1, 1, CG_MediaShader( cgs.media.shaderAdditiveParticleShine ) );
	}
	else
	{
		le->ent.scale = 1.0f;
		CG_ImpactPuffParticles( pos, dir, 8, 1.0f, 1, 1, 1, 1, NULL );
	}
...
void CG_ExplosionsDust( vec3_t pos, vec3_t dir, float radius )
{
	...
	for( i = 0; i < count; i++ )
	{
		...
		le = CG_AllocSprite( LE_INVERSESCALE_ALPHA_FADE, pos, 10, 10,
			1.0f, 1.0f, 1.0f, 1.0f,
			100, 0, 0, 0,
			shader );
