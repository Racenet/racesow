...
void ClientThink( edict_t *ent, usercmd_t *ucmd, int timeDelta )
{
	...
	{
		int nudge;
		int fixedNudge = ( game.snapFrameTime - 1 ) * 0.75; // fixme: find where this nudge comes from.
	...
	ent->s.weapon = GS_ThinkPlayerWeapon( &client->ps, ucmd->buttons, ucmd->msec );
