...
void G_CheckCvars( void )
{
	...
	if( g_warmup_enabled->modified )
	{
		...
		if( !g_warmup_enabled->integer && 
			( GS_MatchState() == MATCH_STATE_WARMUP || GS_MatchState() == MATCH_STATE_COUNTDOWN ) )
			G_Match_LaunchState( MATCH_STATE_PLAYTIME );
	...
	if( g_warmup_timelimit->modified )
	{
		...
		if( GS_MatchState() == MATCH_STATE_WARMUP )
		{
			gs.gameState.longstats[GAMELONG_MATCHDURATION] = (unsigned int)fabs( 60 * 1000 * g_warmup_timelimit->integer );
		}
	...
	if( g_timelimit->modified )
	{
			...
			!GS_MatchExtended() )
		{
			if( g_timelimit->value )
				gs.gameState.longstats[GAMELONG_MATCHDURATION] = (unsigned int)fabs( 60 * 1000 * g_timelimit->value );
			else
...
void G_ClearSnap( void )
{
	...
	for( ent = &game.edicts[0]; ENTNUM( ent ) < game.numentities; ent++ )
	{
		...
		ent->s.effects = ( ent->s.effects & (EF_CARRIER|EF_FLAG_TRAIL|EF_ROTATE_AND_BOB|EF_STRONG_WEAPON) );
	}
