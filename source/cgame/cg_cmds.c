...
static void CG_SC_ChatPrint( void )
{
	...
	if( !name )
		CG_Printf( S_COLOR_GREEN "console: %s\n", text );
	else if( teamonly )
		CG_Printf( S_COLOR_YELLOW "[%s]" S_COLOR_WHITE "%s:" S_COLOR_YELLOW " %s\n", 
			cg.frame.playerState.stats[STAT_REALTEAM] == TEAM_SPECTATOR ? "SPEC" : "TEAM", name, text );
	else
		CG_Printf( "%s" S_COLOR_GREEN ": %s\n", name, text );
...
static void CG_SC_TVChatPrint( void )
{
	...
	CG_Printf( S_COLOR_RED "[TV]" S_COLOR_WHITE "%s" S_COLOR_GREEN ": %s", name, text );
	if( cg_chatBeep->integer )
		trap_S_StartGlobalSound( CG_MediaSfx( cgs.media.sfxChat ), CHAN_AUTO, 1.0f );
}
