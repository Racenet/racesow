...
*/
qboolean CG_CheckBladeAutoAttack( player_state_t *playerState )
{
	...
	if( playerState->POVnum <= 0 || playerState->POVnum > (unsigned)gs.maxclients )
		return qfalse;
	...
	player = &cg_entities[ playerState->POVnum ].current;

	// check for a player to touch
	CG_Trace( &trace, origin, vec3_origin, vec3_origin, end, player->number, CONTENTS_BODY );
	if( trace.ent == -1 )
		return qfalse;

	targ = &cg_entities[ trace.ent ].current;
	...
	if( GS_IsTeamDamage( targ, player ) )
		return qfalse;
...
void CG_PredictMovement( void )
{
	...
	while( ++ucmdExecuted <= ucmdHead )
	{
		...
		if( ucmdReady ) // hmm fixme: the wip command may not be run enough time to get proper key presses
		{
			...
			cg_entities[cg.predictedPlayerState.POVnum].current.weapon = GS_ThinkPlayerWeapon( &cg.predictedPlayerState, pm.cmd.buttons, pm.cmd.msec );
		}
