...
\
	import.MM_UIRequest = CL_MM_UIRequest, \
	import.MM_GetStatus = CL_MM_GetStatus \
)
...
#define DLSSAMPLESMASK ( DLSAMPLESCOUNT-1 )
		...
		uie->DrawConnectScreen( cls.servername, cls.rejected ? cls.rejectmessage : NULL,
			...
			cls.connect_count, cls.demo.playing, backGround );
	}
}
...
*/
void CL_UIModule_MM_UIReply( mm_action_t action, const char *data )
{
	if( uie )
		uie->MM_UIReply( action, data );
}
