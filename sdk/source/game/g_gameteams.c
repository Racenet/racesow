...
edict_t *G_Teams_BestInChallengersQueue( unsigned int lastTimeStamp, edict_t *ignore )
{
	...
	for( e = game.edicts + 1; PLAYERNUM( e ) < gs.maxclients; e++ )
	{
		...
		if( trap_GetClientState( PLAYERNUM( e ) ) < CS_SPAWNED || e->r.client->connecting )
			continue;

		if( e == ignore )
			continue;

		if( e->r.client->queueTimeStamp >= lastTimeStamp )
		{
			if( e->r.client->queueTimeStamp < bestTimeStamp )
			{
				bestTimeStamp = e->r.client->queueTimeStamp;
				best = e;
			}
		}
	}
...
void G_Teams_ExecuteChallengersQueue( void )
{
	...
	while( ent && G_Teams_JoinAnyTeam( ent, qtrue ) )
	{
		...
		ent = G_Teams_BestInChallengersQueue( ent->r.client->queueTimeStamp, ent );
	}
	...
	if( ent->r.client->teamstate.is_coach )
	{
		...
		else if( toremove->s.team == ent->s.team )
		{
			G_PrintMsg( NULL, "%s^7 removed %s^7 from %s team.\n", ent->r.client->netname,
				toremove->r.client->netname, GS_TeamName( toremove->s.team ) );
			Cmd_Spec_f( toremove );
		}
		else
