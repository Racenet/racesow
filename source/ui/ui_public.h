...
typedef struct
{
	...
	void ( *MM_UIRequest )( mm_action_t action, const char *data );
	mm_status_t ( *MM_GetStatus )( void );
} ui_import_t;
...
typedef struct
{
	...
	void ( *DrawConnectScreen )( char *serverName, char *rejectmessage, int downloadType, char *downloadfilename, float downloadPercent, int downloadSpeed, int connectCount, qboolean demoplaying, qboolean backGround );
	...
	void ( *MM_UIReply )( mm_action_t action, const char *data );
} ui_export_t;
