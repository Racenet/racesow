...
void CG_Event_Fall( entity_state_t *state, int parm )
{
	if( ISVIEWERENTITY( state->number ) )
	{
		...
		{
			CG_SexedSound( state->number, CHAN_BODY, "*fall_0", cg_volume_players->value );
			return;
		}
	...
	if( parm > 10 )
	{
		CG_SexedSound( state->number, CHAN_BODY, "*fall_2", cg_volume_players->value );
	...
	{
		CG_SexedSound( state->number, CHAN_BODY, "*fall_1", cg_volume_players->value );
	}
	else
		CG_SexedSound( state->number, CHAN_BODY, "*fall_0", cg_volume_players->value );
...
void CG_Event_Dash( entity_state_t *state, int parm )
{
	...
	case 0: // dash front
		...
		CG_SexedSound( state->number, CHAN_VOICE, va( S_PLAYER_DASH_1_to_2, ( rand()&1 )+1 ), cg_volume_players->value );
		break;
	case 1: // dash left
...
void CG_Event_WallJump( entity_state_t *state, int parm, int ev )
{
	...
	{
		if( ISVIEWERENTITY( state->number ) )
			trap_S_StartGlobalSound( CG_MediaSfx( cgs.media.sfxWalljumpFailed ), CHAN_AUTO, cg_volume_effects->value );
		else
	...
	{
		CG_SexedSound( state->number, CHAN_VOICE, va( S_PLAYER_WALLJUMP_1_to_2, ( rand()&1 )+1 ), cg_volume_players->value );
...
{
	CG_SexedSound( state->number, CHAN_VOICE, va( S_PLAYER_JUMP_1_to_2, ( rand()&1 )+1 ), cg_volume_players->value );
}
...
void CG_EntityEvent( entity_state_t *ent, int ev, int parm, qboolean predicted )
{
	...
	case EV_NOAMMOCLICK:
		if( viewer )
			...
			trap_S_StartFixedSound( CG_MediaSfx( cgs.media.sfxWeaponUpNoAmmo ), ent->origin, CHAN_AUTO, cg_volume_effects->value, ATTN_IDLE );
		break;
