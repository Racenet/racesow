...
static void CG_NewPacketEntityState( entity_state_t *state )
{
	...
	else
	{
		...
		VectorSubtract( state->origin, cent->current.origin, cent->velocity );
		dist = VectorNormalize( cent->velocity );
		if( ( state->modelindex != cent->current.modelindex ) || ( dist > 512 ) || state->teleported )
		{
			cent->serverFrame = -99;
			dist = 0;
		}
		...
		if( dist > 0.0f )
			VectorScale( cent->velocity, dist/(float)( cg.frame.serverTime - cg.oldFrame.serverTime ), cent->velocity );
		else
			VectorClear( cent->velocity );
...
static void CG_SetFramePlayerState( frame_t *frame, int index )
{
...
* CG_ResetItemTimers
*/
static void CG_ResetItemTimers( void )
{
...
{
	qsort( cg_item_timers, cg_num_item_timers, sizeof( cg_item_timers[0] ), CG_CompareItemTimers );
}
...
void CG_SoundEntityNewState( centity_t *cent )
{
	...
	else if( owner && cg_entities[owner].serverFrame == cg.frame.serverFrame )
	{
		...
		if( !cgs.soundPrecache[soundindex] )
		{
			...
			if( cstring && cstring[0] == '*' )
				CG_SexedSound( owner, channel, cstring, 1.0f );
...
void CG_AddEntities( void )
{
	...
	for( pnum = 0; pnum < cg.frame.numEntities; pnum++ )
	{
		...
		case ET_ELECTRO_WEAK:
			...
			CG_AddGenericEnt( cent );
			CG_EntityLoopSound( state, ATTN_STATIC );
			CG_AddLightToScene( cent->ent.origin, 300, 0.8, 0.8, 0.8, NULL );
		...
		if( state->light && !state->linearProjectile )
		{
