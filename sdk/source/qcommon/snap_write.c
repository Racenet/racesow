...
*/
static void SNAP_EmitPacketEntities( ginfo_t *gi, client_frame_t *from, client_frame_t *to, msg_t *msg, entity_state_t *baselines, entity_state_t *client_entities, int num_client_entities )
{
...
*/
static void SNAP_WriteDeltaGameStateToClient( client_frame_t *from, client_frame_t *to, msg_t *msg )
{
	int i;
	short statbits;
	qbyte bits;
...
* SNAP_SnapCullEntity
*/
static qboolean SNAP_SnapCullEntity( cmodel_state_t *cms, edict_t *ent, edict_t *clent, client_frame_t *frame, vec3_t vieworg, qbyte *fatpvs, qbyte *fatphs )
{
	qbyte *areabits;
...
*/
static void SNAP_BuildSnapEntitiesList( cmodel_state_t *cms, ginfo_t *gi, edict_t *clent, vec3_t vieworg, vec3_t skyorg, qbyte *fatpvs, qbyte *fatphs, client_frame_t *frame, snapshotEntityNumbers_t *entsList )
{
	...
	if( clent )
	{
		...
		if( !frame->allentities && clusternum == -1 )
		{
			...
			SNAP_AddEntNumToSnapList( NUM_FOR_EDICT( clent ), entsList );
			return;
		}
	}
...
*/
static void SNAP_FreeClientFrame( client_frame_t *frame )
{
	if( frame->areabits )
	{
		Mem_Free( frame->areabits );
