...
static void UI_UpdateMousePosition( void );
...
static void M_Cache( void )
{
	...
	trap_R_RegisterPic( "gfx/ui/background" );
...
*/
void UI_DrawConnectScreen( char *serverName, char *rejectmessage, int downloadType, char *downloadFilename, 
						  float downloadPercent, int downloadSpeed, int connectCount, qboolean demoplaying, qboolean backGround )
{
	...
	if( backGround )
		trap_R_DrawStretchPic( 0, 0, uis.vidWidth, uis.vidHeight, 0, 0, 1, 1, colorWhite, trap_R_RegisterPic( "gfx/ui/background" ) );
...
void UI_Refresh( unsigned int time, int clientState, int serverState, qboolean demoplaying, qboolean backGround )
{
	...
	if( uis.backGround )
	{
		...
		{
			int num = ( trap_Milliseconds()&1 )+1;
			trap_S_StartBackgroundTrack( va( S_MUSIC_MENU_1_to_2, num ), va( S_MUSIC_MENU_1_to_2, num ) );
