...
static void M_Game_GametypeMenuFunc( menucommon_t *menuitem )
{
	...
	{
		trap_Cmd_ExecuteText( EXEC_APPEND, "gamemenu" );
		M_ForceMenuOff();
	}
}
...
static void M_Game_SpecModeMenuFunc( menucommon_t *menuitem )
{
	if( uis.clientState >= CA_ACTIVE && !uis.demoplaying )
	{
		trap_Cmd_ExecuteText( EXEC_APPEND, "menu_custom chasecam"
			" auto \"chase auto\""
			" objectives \"chase objectives\""
			" powerups \"chase powerups\""
			" score \"chase score\""
			" none \"chase none\""
			" cancel \"\"" );
		M_ForceMenuOff();
