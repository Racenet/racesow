...
void CG_DrawClock( int x, int y, int align, struct mufont_s *font, vec4_t color )
{
	...
	else
	{
		...
		if( duration && ( cg_showTimer->integer != 3 || !GS_InvidualGameType() ) )
		{
...
void CG_UpdatePointedNum( void )
{
	...
	if( cg.predictedPlayerState.stats[STAT_POINTED_PLAYER] )
	{
		...
		pointed_armor_type = ( cg.predictedPlayerState.stats[STAT_POINTED_TEAMPLAYER]>>12 &0xF );
		if( mega )
		{
			pointed_health += 100;
...
{
	static vec4_t alphagreen = { 0, 1, 0, 0 }, alphared = { 1, 0, 0, 0 }, alphayellow = { 1, 1, 0, 0 }, alphamagenta = { 1, 0, 1, 1 };
	centity_t *cent;
	vec4_t tmpcolor;
	...
	for( i = 0; i < gs.maxclients; i++ )
	{
		...
		if( pointed_health )
		{
			...
			alphagreen[3] = alphared[3] = alphayellow[3] = alphamagenta[3] = tmpcolor[3];
...
void CG_DrawLoading( void )
{
	...
	float percent;
	int barwidth, x, y;
	struct shader_s *barShader = trap_R_RegisterPic( "gfx/ui/progressbar" );

	if( !cgs.configStrings[CS_MAPNAME][0] )
		return;

	trap_R_DrawStretchPic( 0, 0, cgs.vidWidth, cgs.vidHeight, 0, 0, 1, 1, colorWhite, trap_R_RegisterPic( "gfx/ui/background" ) );
...
void CG_Draw2DView( void )
{
	...
	if( cgs.demoPlaying || cg.frame.multipov || cgs.tv )
	{
		if( cg.showScoreboard || GS_MatchState() > MATCH_STATE_PLAYTIME ||
		   ( cg.predictedPlayerState.pmove.pm_type != PM_SPECTATOR && cg.predictedPlayerState.stats[STAT_HEALTH] <= 0 ) )
