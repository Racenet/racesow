...
void CG_MoveToTag( vec3_t move_origin,
                   ...
                   const vec3_t space_axis[3],
                   const vec3_t tag_origin,
                   const vec3_t tag_axis[3] )
{
	int i;
	vec3_t tmpAxis[3];
...
void CG_UpdatePlayerModelEnt( centity_t *cent )
{
	...
	if( cent->current.frame ) // animation was provided by the server
	{
		...
		VectorCopy( cent->velocity, cent->animVelocity );
		cent->yawVelocity = 0;
	}
	else
	{  
		...
		cent->lastVelocities[cg.frame.serverFrame&3][0] = cent->animVelocity[0];
		cent->lastVelocities[cg.frame.serverFrame&3][1] = cent->animVelocity[1];
		cent->lastVelocities[cg.frame.serverFrame&3][2] = cent->animVelocity[2];
